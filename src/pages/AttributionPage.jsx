...
import { generateExecutiveSummary, generateRecommendations } from '../services/geminiService';
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-slate-500 mt-1">Brinson-Fachler Methodology</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        <div className="p-5 border-b border-slate-200 bg-slate-50/50 flex items-center justify-between flex-wrap gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-left whitespace-nowrap">
            ...
            <tbody className="divide-y divide-slate-100">
              {data.map((row) => (
                <tr key={row.id} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-2">
                    <input type="text" value={row.name} onChange={(e) => handleChange(row.id, 'name', e.target.value)} className="w-full px-3 py-1.5 border border-slate-200 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" step="0.01" value={row.wp} onChange={(e) => handleChange(row.id, 'wp', e.target.value)} className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-right focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" step="0.01" value={row.rp} onChange={(e) => handleChange(row.id, 'rp', e.target.value)} className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-right focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" step="0.01" value={row.wb} onChange={(e) => handleChange(row.id, 'wb', e.target.value)} className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-right focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                  </td>
                  <td className="px-4 py-2">
                    <input type="number" step="0.01" value={row.rb} onChange={(e) => handleChange(row.id, 'rb', e.target.value)} className="w-full px-3 py-1.5 border border-slate-200 rounded-md text-right focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
                  </td>
                  <td className="px-4 py-2 text-center">
                    <button onClick={() => handleRemoveRow(row.id)} className="text-slate-400 hover:text-red-500 p-1.5 rounded-md hover:bg-red-50 transition-colors">
              ...
              {data.length === 0 && (
                <tr><td colSpan="6" className="px-4 py-8 text-center text-slate-500">No asset classes added. Click "Add Asset Class" to begin.</td></tr>
              )}
            </tbody>
            <tfoot className="bg-slate-50 font-semibold text-slate-700 border-t border-slate-200">
              <tr>
                ...
                <td className="px-4 py-3 text-right bg-blue-50/50">{formatPct(results.portfolioReturn)}</td>
                <td className={`px-4 py-3 text-right ${wbWarning ? 'text-amber-600' : ''}`}>{(results.totalBenchmarkWeight * 100).toFixed(1)}%</td>
                <td className="px-4 py-3 text-right bg-blue-50/50">{formatPct(results.benchmarkReturn)}</td>
                <td></td>
              </tr>
            </tfoot>
          </table>
          ...
          <table className="w-full text-sm text-left whitespace-nowrap">
            <thead className="bg-slate-50 text-slate-500 font-medium border-b border-slate-200">
              <tr>
                ...
                <th className="px-4 py-3 text-right font-bold text-slate-800">Total Contribution</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 font-mono">
              {results.attribution.map((row, i) => (
                <tr key={i} className="hover:bg-slate-50/50 transition-colors">
                  <td className="px-4 py-3 font-sans font-medium text-slate-700">{row.name}</td>
                  <td className={`px-4 py-3 text-right ${row.allocation >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {formatSignedPct(row.allocation)}
                  </td>
                  <td className={`px-4 py-3 text-right ${row.selection >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {formatSignedPct(row.selection)}
                  </td>
                  <td className={`px-4 py-3 text-right ${row.interaction >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {formatSignedPct(row.interaction)}
                  </td>
                  <td className={`px-4 py-3 text-right font-bold ${row.total >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                    {formatSignedPct(row.total)}
                  </td>
                </tr>
              ))}
            </tbody>
            <tfoot className="bg-indigo-50/50 font-bold border-t border-slate-200 font-mono">
              <tr>
                <td className="px-4 py-4 text-right font-sans text-slate-700">Total Active Return:</td>
                <td className={`px-4 py-4 text-right ${results.totals.allocation >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                  {formatSignedPct(results.totals.allocation)}
                </td>
                <td className={`px-4 py-4 text-right ${results.totals.selection >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                  {formatSignedPct(results.totals.selection)}
                </td>
                <td className={`px-4 py-4 text-right ${results.totals.interaction >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
                  {formatSignedPct(results.totals.interaction)}
                </td>
                <td className={`px-4 py-4 text-right text-lg ${results.totals.activeReturn >= 0 ? 'text-emerald-700' : 'text-rose-700'}`}>
      ...
      <div className="bg-slate-100 rounded-2xl p-6 text-slate-600 text-sm border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <strong className="text-slate-700 block mb-1">Allocation Effect</strong>
            <code className="text-xs bg-white px-2 py-1 rounded border border-slate-200 text-blue-600 mb-2 block w-fit">
              (Wp - Wb) x (Rb - Rb_total)
            </code>
            Measures the manager's ability to effectively over-weight outperforming asset classes.
          </div>
          <div>
            <strong className="text-slate-700 block mb-1">Selection Effect</strong>
            <code className="text-xs bg-white px-2 py-1 rounded border border-slate-200 text-emerald-600 mb-2 block w-fit">
              Wb x (Rp - Rb)
            </code>
            Measures the manager's ability to select outperforming securities within each asset class.
          </div>
          <div>
            <strong className="text-slate-700 block mb-1">Interaction Effect</strong>
            <code className="text-xs bg-white px-2 py-1 rounded border border-slate-200 text-amber-600 mb-2 block w-fit">
              (Wp - Wb) x (Rp - Rb)
            </code>
